...
def vowel_output(phone):
    ...
    '''
    vowel_info = [grid.replace(".TextGrid", ""), str(idx_w), word.text, str(vowel_dur), str(idx_p), ph.text, obstruent]
    return(vowel_info)
        ...
        for idx_p, ph in enumerate(phone_seq):
            ...
            if ph.text in vowels and len(phone_seq) == 2: ## don't consider 1-length words
               if idx_p == 0 and phone_seq[idx_p+1].text in obstruents: ## words like "it"
                  ...
                  vdur_info = vowel_output(ph)
                  vdur_info.append(context)
                  vdur_final.append(vdur_info)
...
Title_vowels = "Filename\tWord-Index\tWord\tVowel_Duration\tPhoneme-Index\tPhoneme\tObstruent\tSequence_Type\n"
with open("../indiv_feat_files/V.Sys." + System + ".Duration.Vowels.txt", 'w') as outFile2:
     outFile2.write(Title_vowels)
     for inf in vdur_final:
         outFile2.write('\t'.join(inf))
         outFile2.write('\n')
